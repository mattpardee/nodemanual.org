...
NodeDoc.generateToc = function()
{
	...
	{
		var $this = $(this);
		$this.attr('id', $this.text().replace(/\(.*\)$/gi, '').replace(/[\s\.]+/gi, '-').replace(/('|"|:)/gi, '').toLowerCase());
	...
	function updateNavigation()
	{
		...
		function updateNav($last)
		{
			...
			if ($activeToc.length)
			{
				...
				$('.current-section').remove();
				// GJT: $currentSelection = $last.clone().width($last.width()).addClass('current-section');
				$currentSelection = $last.clone().addClass('current-section');
